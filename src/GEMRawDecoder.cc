...
#ifdef SWITCH_ENDIANESS
    SwitchEndianess(buf, fBuf);
#endif

    int channel_active = 1;
    vector<int> apv;
    apv.clear();
    for(int idata = 0;idata<fBuf;idata++)
    {
	if( ( (buf[idata+1]>>8) & 0xffffff ) == 0x414443 )
	{
	    if(apv.size() > APV_MIN_LENGTH && channel_active==1)
		mAPVRawSingleEvent[nfecID][nadcCh] = apv;
	    apv.clear();
	    nadcCh = buf[idata+1] & 0xff; 
	    nfecID = (buf[idata+2]>>16) & 0xff;
            //cout<<"fec: "<<nfecID<<" adc: "<<nadcCh<<endl;
	    idata+=2;
	    channel_active = 1;
...
unordered_map<int, vector<int> >   & GEMRawDecoder::GetFECDecoded()
{
    return mFecApvEvent;
}
...
unordered_map<int, TH1F* >  & GEMRawDecoder::GetFECAPVRawHisto()
{
    clearFECHistos();
    int index=0;
    int fec_id = 0;
